...
A horse-racing win/place probability model + Streamlit paper-betting dashboard for
...
## Critical honesty rule (do not violate in code or copy)

Beating the market on log-loss is the gate, **but a positive verdict with negative
CLV is paper-only, never a green light to bet.** The v3 LightGBM holdout is exactly
this case (GO on log-loss, CLV ≈ −12%). The headline UI "Win %" is the
**within-race-normalised** probability (`won_prob_normalized`), not the raw model
marginal (which saturates out-of-sample). Keep both facts surfaced, never hidden.
